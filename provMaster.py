import os, sys
from genProvHTML import genProvHTML
from genProvSummaryHTML import genProvSummaryHTML
...
def processWshed(args):
    ...
    wsNameDict = buildDictFrCsv(indir + os.sep + "provNames.txt")
    weblinks = Weblinks()
    ...
    chartDir = ensureDir(os.path.dirname(indir) + os.sep + "output" + os.sep + "provcharts")
    if args.charts:
        icube.genWshedCharts(chartDir, template, "This Province's Ranking")
    ...
    wsHtmlDir = ensureDir(os.path.dirname(indir) + os.sep + "output" + os.sep + "provhtml")
    ...
    for wsid in template.wsidList:
        ...
        f.write(genProvHTML(wsid, template, sourceList, icube, sourceDict, subNameDict, wsNameDict, weblinks, tfDict, popDict))
        f.close()
        progress += 1
    print "           DONE"
    ...
    if args.summary:
        ...
        for wsid in template.wsidList:
            ...
            html = genProvSummaryHTML(wsid, template, sourceList, icube, sourceDict, subNameDict, wsNameDict, popDict)
            f.write(html)
            f.close()   
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--indir', help='Input directory', required=False,
                        default='/home/bveerman/Personal/CAREX/emp2012/GE/htmlGen/prov')

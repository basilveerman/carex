import os, sys
from genCanHTML import genCanHTML
from genCanSummaryHTML import genCanSummaryHTML
...
def processWshed(args):
    ...
    wsNameDict = buildDictFrCsv(indir + os.sep + "canNames.txt")
    weblinks = Weblinks()
    ...
    wsHtmlDir = ensureDir(os.path.dirname(indir) + os.sep + "output" + os.sep + "canhtml")
    ...
    for wsid in template.wsidList:
        ...
        f.write(genCanHTML(wsid, template, sourceList, icube, sourceDict, subNameDict, wsNameDict, weblinks, tfDict))
        f.close()
        progress += 1
    print "           DONE"
    ...
    if args.summary:
        ...
        for wsid in template.wsidList:
            ...
            html = genCanSummaryHTML(wsid, template, sourceList, icube, sourceDict, subNameDict, wsNameDict, 0)
            f.write(html)
            f.close()   

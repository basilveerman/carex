import os, sys
from genEcozoneHTML import genEcozoneHTML
from genEcoSummaryHTML import genEcoSummaryHTML
...
def processWshed(args):
    ...
    wsNameDict = buildDictFrCsv(indir + os.sep + "zone_names.txt")
    weblinks = Weblinks()
    ...
    chartDir = ensureDir(os.path.dirname(indir) + os.sep + "output" + os.sep + "ecozonecharts")
    if args.charts:
        icube.genWshedCharts(chartDir, template, "This Ecozones's Ranking")
    ...
    wsHtmlDir = ensureDir(os.path.dirname(indir) + os.sep + "output" + os.sep + "ecozonehtml")
    ...
    for wsid in template.wsidList:
        ...
        f.write(genEcozoneHTML(wsid, template, sourceList, icube, sourceDict, subNameDict, wsNameDict, weblinks, tfDict, popDict))
        f.close()
        progress += 1
    print "           DONE"
    ...
    if args.summary:
        ...
        for wsid in template.wsidList:
            ...
            html = genEcoSummaryHTML(wsid, template, sourceList, icube, sourceDict, subNameDict, wsNameDict, popDict)
            f.write(html)
            f.close()   
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--indir', help='Input directory', required=False,
                        default='/home/bveerman/Personal/CAREX/emp2012/GE/htmlGen/ecozone')

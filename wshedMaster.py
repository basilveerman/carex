import os, sys
from genWshedHTML import genWshedHTML
from genWshedSummaryHTML import genWshedSummaryHTML
...
def processWshed(args):
    ...
    wsNameDict = buildDictFrCsv(indir + os.sep + "wshedNames.txt")
    weblinks = Weblinks()
    tfDict = buildDictFrCsv(indir + os.sep + "tf.txt")
    
    ## initialize the population aggregator and store resulting dictionary
    pop = population(indir + os.sep + 'population.txt', template)
    popDict = pop.popDict
    ...
    ## generate charts
    chartDir = ensureDir(os.path.dirname(indir) + os.sep + "output" + os.sep + "wscharts")
    if args.charts:
        icube.genWshedCharts(chartDir, template, "This Watershed's Ranking")
    ...
    wsHtmlDir = ensureDir(os.path.dirname(indir) + os.sep + "output" + os.sep + "wshtml")
    ...
    for wsid in template.wsidList:
        ...
        f.write(genWshedHTML(wsid, template, sourceList, icube, sourceDict, subNameDict, wsNameDict, weblinks, popDict, tfDict))
        f.close()
    ...
    if args.summary:
        ...
        for wsid in template.wsidList:
            ...
            html = genWshedSummaryHTML(wsid, template, sourceList, icube, sourceDict, subNameDict, wsNameDict, popDict)
            f.write(html)
            f.close()   
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--indir', help='Input directory', required=False,
                        default='/home/bveerman/Personal/CAREX/emp2012/GE/htmlGen/wshed')

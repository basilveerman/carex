from genHTMLhelper import genTablesorterJava,genShowHideJava,addrollover,genSubTable,genAbout,genFooter,genHTMLHead,insert_thousands_separator,round_float_to_int,format_total_vals,city_disclaimer
from unidecode import unidecode
from resources import ordinal
   
def genHeader(wsid, wsNameDict, wsidDict, totalRankTable, popDict):
    imgref = '../citycharts/' + wsid + '.png'
...
def genMain(wsid, t, totalTable, rankTable, subNameDict, weblinks, tfDict):
    subTable = "	<div id=\"mainContent_wshed\">" + city_disclaimer
    subTable += genSubTable(wsid, t, totalTable, rankTable, subNameDict, weblinks, tfDict, "# CITIES WITH SUBSTANCE")
    subTable += "</div>\n"
    return subTable
...
def genWrapper(wsid, t, sourceList, icube, sourceDict, subNameDict, wsNameDict, weblinks, tfDict, popDict):
...
def genHTMLBody(wsid, t, sourceList, icube, sourceDict, subNameDict, wsNameDict, weblinks, tfDict, popDict):
    htmlbody = "<body>\n"
    htmlbody += genWrapper(wsid, t, sourceList, icube, sourceDict, subNameDict, wsNameDict, weblinks, tfDict, popDict)
    htmlbody += genTablesorterJava(t.subList) + genShowHideJava()
    htmlbody += "</body>\n"
    
    return htmlbody

def genCityHTML(wsid, t, sourceList, icube, sourceDict, subNameDict, wsNameDict, weblinks, tfDict, popDict):

    html = "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">\n\
    <html xmlns=\"http://www.w3.org/1999/xhtml\">\n"
    html += genHTMLHead()
    html += genHTMLBody(wsid, t, sourceList, icube, sourceDict, subNameDict, wsNameDict, weblinks, tfDict, popDict)
    html += "</html>"

import os, sys
from genHealthHTML import genHealthHTML
from genHealthSummaryHTML import genHealthSummaryHTML
...
def processWshed(args):
    ...
    wsNameDict = buildDictFrCsv(indir + os.sep + "healthNames.txt")
    weblinks = Weblinks()
    ...
    chartDir = ensureDir(os.path.dirname(indir) + os.sep + "output" + os.sep + "healthcharts")
    if args.charts:
        icube.genWshedCharts(chartDir, template, "This Region's Ranking")
    ...
    wsHtmlDir = ensureDir(os.path.dirname(indir) + os.sep + "output" + os.sep + "healthhtml")
    ...
    for wsid in template.wsidList:
        ...
        f.write(genHealthHTML(wsid, template, sourceList, icube, sourceDict, subNameDict, wsNameDict, weblinks, tfDict, popDict))
        f.close()
        progress += 1
    print "           DONE"
    ...
    if args.summary:
        ...
        for wsid in template.wsidList:
            ...
            html = genHealthSummaryHTML(wsid, template, sourceList, icube, sourceDict, subNameDict, wsNameDict, popDict)
            f.write(html)
            f.close()   
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--indir', help='Input directory', required=False,
                        default='/home/bveerman/Personal/CAREX/emp2012/GE/htmlGen/health')
